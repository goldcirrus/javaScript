...
function proceed_with_filereading(request, response, pathname) {
    ...
    if (words.length > 1) {                         //.extension exists
          extension = words[words.length - 1];     //extension is the last element in words, but not used here
    } 
	else {                     //If there is no file extension, we assume basename is a directory, and '/index.html' is attached. See 4.2.4
          pathname = pathname + '/index.html';
          extension = 'html';     //extension is not used here
    }
	
	//invoke fs.readFile() to read file's content, and send the content or err message to a client via response to client
