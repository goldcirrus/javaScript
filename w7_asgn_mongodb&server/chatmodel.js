//In chatmodel1.sjs
//functions will be used in file:chatserver1.sjs
//functions ready() connect to mongodb: COMP4620_lzhong, then do CURD operations on mongodb inside ready() as its callbacks definitions in chatserver.sjs
...
/*#############################################*/
function exist(u,callback)          //return true via callback if user already exist, return false if user not exist or can not operate on db
{  
    var collection = db.collection("Users");
    
    collection.find({username:u}).toArray(function(err, resultArray) {  // an example of find(); findOne() can be used instead
        if (err) callback(false);                         //can not make an operation on mongoDB
